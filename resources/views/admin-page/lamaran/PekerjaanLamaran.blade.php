...
@section('content')
<h2 class="font-bold text-2xl mb-2">List Pekerjaan</h2>
...
<div>
    <h1 class="text-1xl mb-5 text-gray-500 font-normal">Silakan pilih pekerjaan yang ingin Anda lihat daftar lamarannya.</h1>

    <form method="GET" action="" class="max-w-3xl mx-auto gap-2 grid grid-cols-1 lg:grid-cols-3">
        <div class="relative w-full">
            <label for="company-name" class="sr-only">Nama Pekerjaan</label>
        ...
        <div class="relative w-full">
            <label for="company-location" class="sr-only">Nama Perusahaan</label>
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 20">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="relative w-full">
            <label for="branch-search" class="sr-only">Lokasi Pekerjaan</label>
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 20">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <section class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        @foreach ($lowongans as $lowongan)
            <div
                ...
                <a href="{{ route('lamaran-admin.detailLamaran', $lowongan->id) }}">
                    <div class="flex justify-center md:justify-end -mt-16">
                        <img class="w-20 h-20 object-cover rounded-full border-2 border-indigo-500"
                            src="{{ asset('storage/gambars/' . $lowongan['perusahaan']['logo']) }}">
                    </div>
                    <div>
                        <h2 class="text-gray-800 text-3xl font-semibold">{{ $lowongan->nama_pekerjaan }}</h2>
                        <p class="mt-2 text-gray-600">{!! Str::limit($lowongan->about_job, 200) !!}</p>
                    </div>
                    <div class="flex justify-between mt-4">
                        <p class="mt-2 text-gray-400 font-bold">{{ $lowongan->perusahaan->nama_perusahaan }}</p>
                        <a href="#" class="text-xl font-medium text-indigo-500">{!! Str::limit($lowongan->lokasi, 10) !!}</a>
                    </div>
                </a>
            </div>
        @endforeach
